...
| **GraphRAG + VisRAG** | 同时召回课程概念路径、文本证据和图像切片证据，避免知识边界缺失，支持网页多模态富媒体原文直连与 NotebookLM 导读自动提炼（3s 摘要、5大看点、3大 FAQ） |
| **DRAG 证据辩论** | 正方、反方、法官三角色过滤弱相关、矛盾或污染证据，降低大模型幻觉，包含国内热门分词消除歧义（DeepSeek与deepin歧义）防护机制 |
| **InstructRAG 约束生成** | 每个生成 Agent 都绑定图谱路径、干净证据和学生画像进行按需适配 |
| **双曲流形对齐** | Poincaré 非欧几何模型，检查讲义、代码、导图、视频脚本之间是否逻辑一致并防冲突，引入 $384\times384$ 维空间投影矩阵与 $O(1)$ MLP 极速拟合代理 |
| **卡尔曼滤波去噪与情感感知** | 基于 EKF 对答题与会话时序进行滤波平滑认知掌握度，心流防重置计数与挫败感时序生理曲线绘制 |
| **知识追踪 DKT & BKT** | 整合贝叶斯知识追踪（BKT）与在线微调的深度知识追踪（DKT）双轨计算掌握概率 |
| **自适应路径规划 (A\*)** | 叶子概念智能推荐，A* 算法生成通关路径，首创活跃学习前沿滑动视窗（2已学 + 1当前 + 3未学）及 terminal 终点桥接，展示预期时长与认知负荷 |
| **自适应学习策略与 CAT** | 支持自适应测试（CAT），基于最近三轮答题判定能力，集成 Anki SM-2 间隔复习算法与打卡签到统计 |
| **错题本错因病理诊断** | 将错因精细归类为学习病理（概念未掌握、熟练度不足、粗心/笔误），提供 misconceptions 饼图及 Top 10 薄弱概念分析 |
| **高可用控制与沙箱判题** | 内置令牌桶流控限流器与熔断机制，隔离代码沙箱内置 Axes3D 绘图白名单及资源强熔断控制 |
...
| **资源工厂** | 视频推荐官 (`director`) | 自动检索匹配教学微课视频，生成个性化推荐描述与音视频播放直链 |
...
9.  **`quiz_items` (题库种子表)**：存储系统预置的 750 道真实专家级多维自适应评测（IRT/CAT）测试题目及解析。
10. **`web_search_history` (搜索历史表)**：存储联网 DuckDuckGo 检索及 URL 抓取解析后的富文本预览。
11. **`code_executions` (代码沙箱运行表)**：记录在隔离沙箱中执行的代码段、时长、以及控制台输出。
12. **`wrong_questions` (错题表)**：FK 关联 `quiz_records` 与母表，存储因答错归档的薄弱点，提供错因分类（wrong_reason_category）。
13. **`checkin_logs` (打卡日志表)**：记录打卡日期及复习时长，用以绘制打卡时间曲线。
14. **`arxiv_cache` (Arxiv 检索缓存表)**：以 `query_hash` 为键缓存学术文献摘要，防止官方高频限流（HTTP 429）。
...
edumatrix-main/
├── app/                        # FastAPI 后端核心配置
│   ├── database.py             # 14张 ORM 表定义与 SQLite WAL 事务控制
│   ├── crud.py                 # 学生画像读取、复习计划管理等 CRUD 物理层
│   ├── auth.py                 # OAuth2 JWT 安全验证令牌发放
│   ├── main.py                 # 路由总挂载、启动钩子及异常过滤中间件
│   ├── agents/                 # 后端内置子智能体逻辑
│   └── utils/                  # 包含强化学习(rl_planner)和公式提取工具
├── frontend/                   # Vue 前端实现
│   ├── src/                    # 源代码
│   │   ├── components/         # MasteryRadar、AnkiFlashcard 等 16 个核心可视化组件
│   │   ├── views/              # Chat、WrongQuestionBook 等 16 个核心功能页面
│   │   ├── api/                # 接口契约定义
│   │   ├── router/             # 路由守卫与全路径指纹映射
│   │   ├── App.vue             # 全局布局与 404 自适应图片避障入口
│   │   ├── main.js             # Vue 初始化引导
│   │   └── style.css           # 全局 CSS 变量与 Tailwind 配置
│   └── package.json            # 前端依赖配置
├── scripts/                    # 开发及部署辅助脚本
│   ├── seed_students.py        # 教师端一键初始化班级演示数据
│   ├── ingest_to_faiss.py      # 多模态课件切片与 FAISS 向量数据库构建
│   └── ...
├── tests/                      # 后端单元与集成测试目录
│   ├── test_goal_recommendations.py # A* 自适应路径与叶子推荐测试用例
│   ├── test_recommendation_engine.py # 战术路线与资源编译引擎测试用例
│   └── ...
├── agent_swarm.py              # 1+3+5 智能体架构并发调度 Swarm 控制器
├── bkt_engine.py               # BKT（贝叶斯追踪）与 DKT 时序网络动态更新引擎
├── drag_debate.py              # DRAG 证据抗干扰辩论清洗引擎（Prover-Challenger-Judge）
├── rag_engine.py               # HybridRAG 混合检索与 ColPali VisRAG 视觉检索管道
├── instruct_rag.py             # 融合知识路径、干净证据与画像约束的 LLM 提示词生成器
├── manifold_alignment.py       # 双曲流形仿射对齐与委员会决策合成一致性校验器
├── learning_strategy.py        # 庞加莱非欧几何映射与 EKF 卡尔曼平滑去噪模型
├── models.py                   # 10维状态、心流指标与 evidence 在线数据实体定义
├── concurrency.py              # 高可用流控限流器与熔断器 (AsyncWorkerPool)
├── config.py                   # Pydantic v2 配置载入类
├── llm_client.py               # 统一大模型客户端接口与离线模板小模型降级防线
├── document_parser.py          # 多模态文档解析器（PDF/Word/PPT/LaTeX 提炼）
├── embedding_models.py         # 向量化与重排模型本地/API 接入管理器
├── vector_store.py             # 向量索引数据接口规范
├── vector_store_faiss.py       # FAISS 向量库构建与检索
├── note_engine.py              # 学习日志总结与反思自动提炼引擎
├── observability.py            # 全链路 APM 监控与可观测性 Telemetry
├── mirt_engine.py              # 多维项目反应理论（MIRT）算法引擎
├── multimodal_alignment.py     # 视觉特征与学术文本空间跨模态对齐
├── anki_engine.py              # 闪卡生成器
├── export_pdf.py               # A4 打印级 PDF 生成器
├── run.py                      # 后端服务 Uvicorn 启动入口
├── start.bat                   # 前后端免环境一键双击启动器
├── pull.bat                    # 一键 rebase 同步拉取队友代码
├── sync.bat                    # 3秒极速本地代码一键推送 GitHub
├── test_edumatrix.py           # 80 项核心 pytest 自动化集成测试用例
├── requirements.txt            # Python 依赖包清单
├── AGENTS.md                   # 智能体开发规范与系统业务模块清单
└── README.md                   # 本说明文件
```
