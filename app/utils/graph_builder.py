...
def _rule_based_triplets(chunk_text: str) -> tuple[Triplet, ...]:
    ...
    patterns: list[tuple[str, str, int, int]] = [
        ...
        (r"(?:学习\s*)?([一-鿿\w]{1,20}?)\s*是\s*([一-鿿\w]{1,20}?)\s*的(?:前置|基础|前提条件|先决条件)", "PREREQUISITE_OF", 1, 2),
        # 句式 2: 学习 A 之前必须先掌握 B  → B 是 A 的前置
        (r"学习\s*([一-鿿\w]{1,20}?)\s*(?:之前|以前|前).*?(?:先|必须|需要).*?(?:掌握|理解|学会|学完)\s*([一-鿿\w]{2,20})", "PREREQUISITE_OF", 2, 1),
        # 句式 3: A 依赖于 B
        ...
        (r"在\s*([一-鿿\w]{1,20}?)\s*(?:之后|后面|之后通常)\s*(?:使用|接|连接|拼接)\s*([一-鿿\w]{2,20})", "PREREQUISITE_OF", 1, 2),
        # 句式 7: A 通过 B (实现/完成/解决)
        (r"([一-鿿\w]{1,20}?)\s*通过\s*([一-鿿\w]{1,20}?)\s*(?:实现|完成|计算|解决|得到)", "PREREQUISITE_OF", 2, 1),
        # 句式 8: 必须先(掌握/理解) A 才能(学习/理解) B
        (r"必须先\s*(?:掌握|理解|学会)\s*([一-鿿\w]{1,20}?)\s*才能\s*(?:学习|理解|掌握|开始)\s*([一-鿿\w]{2,20})", "PREREQUISITE_OF", 1, 2),
        # 兜底: A 是 B 的重要组成部分 → "是...的" 后置模式（非 exact 匹配）
        (r"([一-鿿\w]{1,20}?)\s*是\s*([一-鿿\w]{1,20}?)\s*(?:中|里|内部|体系)?的(?:重要|核心|关键|基础|根本)(?:组成)?部分", "PREREQUISITE_OF", 1, 2),
    ]
