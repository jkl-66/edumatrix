...
<template>
  <!-- 浮动图标（最小化状态） -->
  <div v-if="minimized"
    class="fixed bottom-6 right-6 z-50 w-12 h-12 rounded-full bg-gradient-to-br from-purple-500 to-indigo-600 shadow-lg flex items-center justify-center cursor-pointer hover:scale-110 transition-all hover:shadow-xl"
  ...
  <!-- 主面板 -->
  <div v-else
    class="fixed bottom-6 right-6 z-50 w-80 max-h-[70vh] bg-gray-900/95 backdrop-blur-md border border-gray-700/50 rounded-2xl shadow-2xl flex flex-col overflow-hidden transition-all duration-200">
    ...
    <!-- Content -->
    <div v-if="!collapsed" class="flex-1 overflow-y-auto p-3 space-y-3 scrollbar-thin">
